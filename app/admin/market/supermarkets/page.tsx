...
export default function AdminSupermarketsPage() {
  ...
  const [categoriesCsv, setCategoriesCsv] = useState("Frozen, Snacks, Canned");
  ...
  const resetForm = () => {
    ...
    setAddress("");
    setImageUrl("");
    setCategoriesCsv("Frozen, Snacks, Canned");
    setPriceLevel(2);
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      if (mode === "create") {
        await addDoc(collection(db, "marketSupermarkets"), {
          ...payload,
          createdAt: serverTimestamp(),
        });
        setStatus("Supermarket added ✅");
      } else {
        if (!editingId) return;
        await updateDoc(doc(db, "marketSupermarkets", editingId), payload);
        setStatus("Supermarket updated ✅");
      }
      ...
      setError("Save failed. Check rules (admin) and try again.");
    }
  };

  const remove = async (id: string) => {
    const ok = confirm("Delete this supermarket? This cannot be undone.");
    if (!ok) return;
    ...
    try {
      await deleteDoc(doc(db, "marketSupermarkets", id));
      setStatus("Supermarket deleted 🗑️");
  ...
  if (authLoading) {
    return <p className="text-sm text-[var(--kh-text-secondary)]">Checking admin…</p>;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--kh-text-secondary)]">
              Add / edit Pinoy supermarkets and keep them searchable by city.
            </p>
          </div>
      ...
      <section className="kh-card card-hover">
        ...
        {loading ? (
          ...
          <p className="mt-3 text-sm text-[var(--kh-text-secondary)]">
            No supermarkets found. Add your first one 🔥
          </p>
        ) : (
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl rounded-3xl border border-[var(--kh-border)] bg-[var(--kh-bg-card)] p-5 shadow-[var(--kh-card-shadow)]">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--kh-text-muted)]">
                  {mode === "create" ? "Create supermarket" : "Edit supermarket"}
                </p>
                <h3 className="mt-1 text-lg font-semibold text-[var(--kh-text)]">
                  {mode === "create" ? "Add a new store" : "Update details"}
                </h3>
              </div>
              <button
            ...
            <form onSubmit={submit} className="mt-4 grid gap-3 text-sm">
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Store name *">
                  <input value={name} onChange={(e) => setName(e.target.value)} required className="kh-input" />
                </Field>
                <Field label="City *">
                  <select value={city} onChange={(e) => setCity(e.target.value)} className="kh-input">
                    {CITY_OPTIONS.map((c) => (
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Area (optional)">
                  <input value={area} onChange={(e) => setArea(e.target.value)} className="kh-input" />
                </Field>
                <Field label="Hours (optional)">
                  <input value={hours} onChange={(e) => setHours(e.target.value)} className="kh-input" />
                </Field>
              </div>

              <Field label="Address (optional)">
                <input value={address} onChange={(e) => setAddress(e.target.value)} className="kh-input" />
              </Field>

              <Field label="Image URL (optional)">
                <input value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} className="kh-input" />
              </Field>

              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Categories (comma separated)">
                  <input value={categoriesCsv} onChange={(e) => setCategoriesCsv(e.target.value)} className="kh-input" />
                </Field>

                <Field label="Price level (1-3)">
                  <select value={priceLevel} onChange={(e) => setPriceLevel(Number(e.target.value))} className="kh-input">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Phone (optional)">
                  <input value={phone} onChange={(e) => setPhone(e.target.value)} className="kh-input" />
                </Field>
                <Field label="Google Maps link (optional)">
                  <input value={mapUrl} onChange={(e) => setMapUrl(e.target.value)} className="kh-input" />
                </Field>
              </div>

              <div className="grid gap-3 md:grid-cols-2">
                <Field label="Instagram URL (optional)">
                  <input value={instagram} onChange={(e) => setInstagram(e.target.value)} className="kh-input" />
                </Field>
                <Field label="Facebook URL (optional)">
                  <input value={facebook} onChange={(e) => setFacebook(e.target.value)} className="kh-input" />
                </Field>
              </div>

              <Field label="Description (optional)">
                <textarea rows={3} value={description} onChange={(e) => setDescription(e.target.value)} className="kh-input" />
              </Field>
            ...
            {imageUrl && (
              <div className="mt-4">
                <p className="text-[11px] font-semibold text-[var(--kh-text-muted)]">Preview</p>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={imageUrl} alt="Preview" className="mt-2 h-44 w-full rounded-2xl object-cover border border-[var(--kh-border)]" />
              </div>
            )}
          </div>
        </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1">
      <label className="text-[11px] font-medium text-[var(--kh-text-secondary)]">{label}</label>
      {children}
    </div>
  );
}
